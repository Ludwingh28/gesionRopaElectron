...
function requireNamedPlaceholders() {
  ...
  function createCompiler(config) {
    ...
    if (config.cache !== false && !cache) {
      cache = new (/* @__PURE__ */ requireLruCache())({ max: ncache });
    }
...
function requirePool$2() {
  if (hasRequiredPool$2) return pool$3;
  hasRequiredPool$2 = 1;
  const process2 = require$$0$3;
  const SqlString2 = requireSqlstring();
  const EventEmitter = require$$0$4.EventEmitter;
...
function requirePool$1() {
  if (hasRequiredPool$1) return pool$2;
  hasRequiredPool$1 = 1;
  const EventEmitter = require$$0$4.EventEmitter;
  const makeDoneCb = requireMake_done_cb();
  const PromisePoolConnection = requirePool_connection$1();
...
async function executeQuery(sql, params) {
  ...
  try {
    ...
    const [rows] = await connection2.query(sql, params);
    return rows;
  } catch (error) {
